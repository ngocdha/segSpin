...
def load_mnist_resized(size=8, index=0):
...
def load_digits_8x8(index=0, invert=False):
    """
    Load one sample from sklearn's built-in digits dataset.
    Returns an 8x8 float image scaled to [0,255].

    Parameters
    ----------
    index : int
        Which digit sample to load.
    invert : bool
        If True, invert intensities.
    """
    digits = load_digits()
    X = digits.images  # shape (n_samples, 8, 8), values usually in [0,16]

    img = X[index].astype(float)
...
def exact_observables_sector(Hk, basis, beta, n, label_site):
    ...
    Zk_eigs = np.sum(w)
    print("trace diff =", abs(Zk_eigs - Zk))
...
if __name__ == "__main__":
    ...
    # MNIST_INDEX = 0
    # img = load_mnist_resized(size=SIZE, index=MNIST_INDEX)
    # H, W = img.shape

    DIGIT_INDEX = 0
    img = load_digits_8x8(index=DIGIT_INDEX, invert=False)
    H, W = img.shape
    ...
    # out_dir = (
    #     f"out_exact_mnist{SIZE}x{SIZE}_"
    #     f"idx{MNIST_INDEX}_"
    #     f"{PRUNE_MODE}_"
    #     f"win{WINDOW}_"
    #     f"eps{EPS}_"
